...
// Add Zen Tabs styles
if (theme_get_setting('henric_tabs')) {
  drupal_add_css( drupal_get_path('theme', 'basic') .'/css/tabs.css');
}

function henric_preprocess_page(&$vars, $hook) {
...
function henric_preprocess_node(&$vars) {
  // Add a striping class.
  $vars['classes_array'][] = 'node-' . $vars['zebra'];

  // Merge first/last class (from henric_preprocess_page) into classes array of current node object.
  $node = $vars['node'];
...
function henric_preprocess_block(&$vars, $hook) {
  // Add a striping class.
 ...
 */
function henric_breadcrumb($variables) {
  $breadcrumb = $variables['breadcrumb'];
  // Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('henric_breadcrumb');
  if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {

    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('henric_breadcrumb_home');
    ...
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = theme_get_setting('henric_breadcrumb_separator');
      $trailing_separator = $title = '';
      if (theme_get_setting('henric_breadcrumb_title')) {
        $item = menu_get_item();
      ...
      elseif (theme_get_setting('henric_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }
 ...
 */	
function henric_id_safe($string) {
 ...
 * 
 */
function henric_menu_link(array $variables) {
  ...
  $element['#attributes']['class'][] = henric_id_safe($element['#title']);
 ...
 */
function henric_preprocess_menu_local_task(&$variables) {
 ...
 */
function henric_menu_local_tasks(&$variables) {  
  $output = '';
